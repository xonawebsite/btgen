...
--three       Include Three.js
  ...
  createFiles: function(projectName, options = []){
    ...
    fs.writeFileSync(`./${projectName}/index.html`, html);
    
    if (!options.includes('--no-server')){
      let robots = robotsTXT();
      fs.writeFileSync(`./${projectName}/robots.txt`, robots);
      let humans = humansTXT();
      fs.writeFileSync(`./${projectName}/humans.txt`, humans);
    }
    ...
    if (!options.includes('--no-css')){
      let css = gameCSS();
      fs.writeFileSync(`./${projectName}/css/master.css`, css);
    ...
    if (!options.includes('--no-js')){
      if (!options.includes('--phaser') || !options.includes('--three') || !options.includes('--babylon') || !options.includes('--aframe')){
        let js = gamebase();
        fs.writeFileSync(`./${projectName}/js/game.js`, js);
      }
      
      let devTools = gameDevTools();
      fs.writeFileSync(`./${projectName}/js/devTools.js`, devTools);
    }
    ...
    if (options.includes('--phaser')){
      ...
      fs.writeFileSyncSync(`./${projectName}/package.json`, json);
    }
