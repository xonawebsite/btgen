...
Available Options
-------------------------------------------------------------------------------
--angular     Include angularJS support
--react       Include react and reactDOM
--vue         Include vue.js
--game        Include game template
...
--no-jq       Exclude JQuery
  ...
  createFiles: function(projectName, options = []){
    ...
    if (options.includes('--react')){
      ...
      js = reactJS();
      fs.writeFileSync(`./${projectName}/app/scripts/main.js`, js);

      react(true, `./${projectName}/app/scripts/vendor`);
      react(false, `./${projectName}/app/scripts/vendor`);
      reactDOM(true, `./${projectName}/app/scripts/vendor`);
      reactDOM(false, `./${projectName}/app/scripts/vendor`);
    }

    if (options.includes('--vue')){
      home = html.vueHTML(projectName);
      fs.writeFileSync(`./${projectName}/app/index.html`, home);

      js = vueJS();
      fs.writeFileSync(`./${projectName}/app/scripts/main.js`, js);
            
      vue(true, `./${projectName}/app/scripts/vendor`);
      vue(false, `./${projectName}/app/scripts/vendor`);
    }

    if (options.includes('--game')){
      home = html.gameHTML(projectName);
      fs.writeFileSync(`./${projectName}/app/index.html`, home);

      let gdd = text.simpleGDD();
      fs.writeFileSync(`./${projectName}/GDD.md`, gdd);

      js = javascript.gamebase();
      fs.writeFileSync(`./${projectName}/app/scripts/game.js`, js);

      let devTools = gameDevTools();
      fs.writeFileSync(`./${projectName}/app/scripts/devTools.js`, devTools);

      gameMusic(`./${projectName}/app/assets/audio`);
    }
